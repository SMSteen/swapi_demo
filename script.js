...
function tellTheStory(myPersonData, myPlanetData,mySpeciesData){
	...
	//build the story
		var story = `
		<span>${myPersonData.character}</span> was born in <span>${myPersonData.birthYear}</span> and hails from <span>${myPlanetData.planetName}</span>, a planet where the climate is ${myPlanetData.climate}, the terrain is ${myPlanetData.terrain} and water sources are ${waterAmt}. ${genderRef} has <span>${myPersonData.eyeColor} eyes</span>, <span>${myPersonData.hairColor} hair</span>, and <span>${myPersonData.skinColor} skin</span>. ${genderRef} weighs <span>${myPersonData.weight} pounds</span>, and at <span>${myPersonData.height}</span>, he is shorter than the average <span>${mySpeciesData.species}</span>, a race of <span>${mySpeciesData.beingType} beings</span>, capable of higher intelligence, that commonly speak <span>${mySpeciesData.language}</span>.
	`
